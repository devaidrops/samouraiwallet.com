...
export default function ReviewPage({ review, generalOption, host }) {
  ...
  useEffect(() => {
    if (review.allow_thread && review.threaded_comment) {
      ...
      fetchComment();
      setRndPairs(review.random_avatar_name_pair ? JSON.parse(review.random_avatar_name_pair) : []);
    }
  }, [review]);
  ...
  const linkConfig = useMemo(() => {
    const isInternalLink = review.external_link && (review.external_link.split("/")[2] === host || !(review.external_link.startsWith("https://") || review.external_link.startsWith("http://")));
    ...
    <>
      ...
      {generalOption?.bottom_icons?.length > 0 && (
        <div className="bottom-mobile-menu">
          <div className="inner">
            {generalOption.bottom_icons[0]?.icon && generalOption.bottom_icons[0]?.label && (
      ...
      <div className="content-inside brokers-show">
              ...
              <div className="middle-part">
                ...
                <div className="card-date-date">
                  ...
                  <div className="date-text">
                    Актуально на {review.publishedAt && moment(review.publishedAt).format("MMM YYYY")}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="top-triggers-wrapper">
              <div className="triggers-line">
                {triggerValues.map((item) => (
                  item?.value && item?.icon && item?.label && (
        ...
        {generalOption.company_info_widgets?.length > 0 && review.company_info?.length > 0 && (
          <>
            ...
            >
              {generalOption.company_info_widgets.map((item, index) => {
                const companyInfoItem = review.company_info[index];
                if (!companyInfoItem) return null;
                    ...
                    <div className="unit-info">
                      <div className="unit-value" itemProp="foundingDate">
                        {companyInfoItem.link ? (
                          <Link href={companyInfoItem.link}>
              ...
              <div className="counts">
                {review.summary?.length > 0 &&
                  review.summary.map((item) =>
                    item?.id && item?.title != null && item?.rating != null ? (
                          ...
                          <div className="line-wrapper">
                            <div className="line">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, (100 / 5) * item.rating)}%`,
                                }}
                              ></div>
                            </div>
                          </div>
...
<div
  className={clsx(
    ...
    ((review?.comments ?? []).filter((c) => !c.featured).length < 5 || loadMore) &&
      "full-list"
  )}
>
                {review.allow_thread && threadedComment && (
                  <Fragment key={10000}>
                    <CommentBlock
                      ...
                      rName={rndPairs?.[0]?.name}
                      rAvatar={rndPairs?.[0]?.avatar}
                      threaded
                    />
                    {threadedComment.comments?.length > 0 &&
                      threadedComment.comments.map((childComment, k1) =>
                        childComment?.id ? (
                          <CommentBlock
                            ...
                            handleCommentUpVote={handleCommentUpVote}
                            rName={rndPairs?.[k1 + 1]?.name}
                            rAvatar={rndPairs?.[k1 + 1]?.avatar}
                            threaded
                          />
                        ) : null
                      )}
  ...
  .map((comment) =>
    comment?.id ? (
      <Fragment key={comment.id}>
        <CommentBlock
          comment={comment}
          className={comment.comments?.length ? "comments-list-item_has-answer" : ""}
          handleCommentUpVote={handleCommentUpVote}
        />
...
{(review?.comments ?? []).filter((c) => c && !c.featured).length > 4 &&
  !loadMore && (
    <button
